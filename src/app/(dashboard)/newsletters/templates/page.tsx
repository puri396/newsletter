...
const CATEGORY_COLORS: Record<string, string> = {
  ...
  Digest: "bg-indigo-500/10 text-indigo-300 border-indigo-500/20",
  Onboarding: "bg-emerald-500/10 text-emerald-300 border-emerald-500/20",
  Events: "bg-amber-500/10 text-amber-300 border-amber-500/20",
  Marketing: "bg-fuchsia-500/10 text-fuchsia-300 border-fuchsia-500/20",
  Content: "bg-sky-500/10 text-sky-300 border-sky-500/20",
  Health: "bg-teal-500/10 text-teal-300 border-teal-500/20",
  Business: "bg-slate-500/10 text-slate-300 border-slate-500/20",
  Education: "bg-violet-500/10 text-violet-300 border-violet-500/20",
  Lifestyle: "bg-orange-500/10 text-orange-300 border-orange-500/20",
  Technology: "bg-blue-600/10 text-blue-200 border-blue-600/20",
  Essay: "bg-stone-500/10 text-stone-300 border-stone-500/20",
  News: "bg-yellow-500/10 text-yellow-300 border-yellow-500/20",
};

function CategoryBadge({ category }: { category: string }) {
  const cls = CATEGORY_COLORS[category] ?? "bg-zinc-700/40 text-zinc-300 border-zinc-600/30";
  return (
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h2 className="text-lg font-semibold text-zinc-50">Newsletter Templates</h2>
          <p className="mt-1 text-sm text-zinc-400">
            Pick a template, customise the content, then save as a draft or share directly.
          </p>
        </div>
        <Link
          href="/epic"
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        ...
        <article className="group flex flex-col rounded-xl border border-dashed border-zinc-700 bg-zinc-950/40 p-5 transition hover:border-zinc-500 hover:bg-zinc-900/50">
          ...
          <Link
            href="/newsletters/templates/blank"
        ...
        {NEWSLETTER_TEMPLATES.map((tpl) => (
          ...
          >
            ...
            <Link
              href={`/newsletters/templates/${tpl.id}`}
